"""Trading decision engine and portfolio optimization."""
...
class PortfolioOptimizer:
    ...
    def _get_conversion_rate(
        ...
        """
        # Try direct pair first (e.g., BTC-USD)
    ...
    def generate_trade_opportunities(
        ...
        for from_currency, balance in holdings.items():
            ...
            for to_currency in set([c for pair in available_pairs for c in pair.split('-')]):
                ...
                expected_return_pct = float((to_value - from_value) / from_value) if from_value > 0 else 0.0
                ...
                confidence = self._calculate_confidence(expected_return_pct, volatility)
                ...
                trade = TradeDecision(
                    ...
                    reason=f"Potential {expected_return_pct:.2%} gain with {volatility:.2f} volatility"
                )
